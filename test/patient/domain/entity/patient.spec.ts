...
test("Should create a new patient with cpf", async () => {
  const patient = new Patient(uuidv4(), 'Cezar Zaleski', new Date('2000-11-23'), Sex.Masculine, HospitalizationStatus.Admitted, '994.499.110-45', );
  expect(patient).toBeDefined()
});

test("Should throw exception empty param fullname", async () => {
  expect(() => new Patient(uuidv4(), undefined, new Date('2000-11-23'), Sex.Masculine, HospitalizationStatus.Admitted, '994.499.110-45'))
...
test("Should throw exception empty param sex", async () => {
  expect(() => new Patient(uuidv4(), 'fullname',new Date('2000-11-23'), undefined, HospitalizationStatus.Admitted, '994.499.110-45'))
    .toThrow(new EmptyParamError('sex'))
});
test("Should throw exception empty param hospitalizationStatus", async () => {
  expect(() => new Patient(uuidv4(), 'fullname',new Date('2000-11-23'), Sex.Masculine, undefined, '994.499.110-45'))
    .toThrow(new EmptyParamError('hospitalizationStatus'))
});
test("Should throw exception invalid param sex", async () => {
  expect(() => new Patient(uuidv4(), 'fullname',new Date('2000-11-23'), 'ssss', HospitalizationStatus.Admitted, '994.499.110-45'))
    .toThrow(new InvalidParamError('sex'))
});
test("Should throw exception invalid param hospitalizationStatus", async () => {

  expect(() => new Patient(uuidv4(), 'fullname',new Date('2000-11-23'), Sex.Female, 'adasdasdsad', '994.499.110-45'))
